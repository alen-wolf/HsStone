package com.example.hstone.classOverview.mage
...
import com.example.hstone.databinding.FragmentMageBinding
...
class MageFragment : Fragment() {
    ...
    private  val viewModel: MageViewModel by lazy {
        ViewModelProvider(this).get(MageViewModel::class.java)
    }

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
                              savedInstanceState: Bundle?): View? {
        val binding = FragmentMageBinding.inflate(inflater)

        binding.lifecycleOwner=this

        binding.mageViewModel= viewModel
        ...
        viewModel.navigateToSelectedCard.observe(viewLifecycleOwner, Observer {
            if ( null != it ) {
                this.findNavController().navigate(
                    MageFragmentDirections.actionMageFragmentToDetailFragment(it)
                )
        ...
        binding.floatingActionButton.setOnClickListener {
            val action = MageFragmentDirections.actionMageFragmentToFavouriteCards()
